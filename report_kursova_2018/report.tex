...
\chapter{2. Обчислення за допомогою ієрархічних матриць.}
	...
	\par Нас цікавить проблема граничного інтегралу, тобто множина $\Omega$ буде підмноговидом. В нашому випадку,$\Omega$ є одновимірним підмноговидом $\mathbb{R}^2$ тобто є кривою. Введемо означення інтегралу на кривій.
	\subsection{Криволінійний інтеграл}
	\hspace{0.8cm} Нехай $\gamma:[0,1]\rightarrow\mathbb{R}^2$ - неперервно диференційована функція, ін'єктивна в $[0,1]$. Означимо її діапазон як $\Gamma:=\gamma([0,1])$.
	\par Розглядаємо $u\in C(\Gamma)$.
	\par Схоже до побудови інтеграла Рімана, розгрядаємо поділ $0=x_0<x_1<...<x_n=1$ інтервала $[0,1]$ і суму
	$$I_x:=\sum_{i=1}^{n}u(\gamma(x_i))\|\gamma(x_i)-\gamma(x_{i-1})\|$$
	\begin{Lem}
		Нехай $\epsilon\in \mathbb{R}_{>0}$. Тоді $\exists\delta\in\mathbb{R}_{>0}$ таке що для будь-якого поділу  $0=x_0<x_1<...<x_n=1$ з
		$$x_i-x_{i-1}<\delta\text{   } (i\in\{1,...,n\})$$
		виконується
		$$|I_x-\int_{0}^{1}u(\gamma(y))\|\gamma^\prime(y)\|dy|\le\epsilon$$
	\end{Lem}
	\par Тепер ми можемо означити криволінійний інтеграл:\newline
	нехай $(\gamma_i)_{i=1}^m$ - кортеж ін'єктивних функцій в $C^1([0,1],\mathbb{R}^2)$. Покладемо $\Gamma_i:=\gamma_i([0,1])$ для всіх $i\in \{1,...,m\}$. Криволінійний інтеграл над кусково-неперервною кривою $\Gamma:=\bigcup_{i=1}^m\Gamma_i$ задається наступним чином
	$$\int_{\Gamma}u(x)dx:=\sum_{i=1}^{m}\int_{0}^{1}u(\gamma_i(y))\|\gamma_i^\prime(y)\|dy$$

import de.lab4inf.math.functions.CosineIntegral.ci
import de.lab4inf.math.functions.Sine
import de.lab4inf.math.functions.SineIntegral
import de.lab4inf.math.functions.SineIntegral.si
import kotlin.math.*
...
fun fpervisna3(x:Double):Double{
    var sum = 0.0
    if(x!=1.0)
        sum += -(1.0/PI)*(cos(PI*x)*ln(abs(1-x))-cos(PI*x)*ci(PI*(1-x)))
    sum+=-(1.0/PI)*(sin(PI*x)*si(PI*(1-x)))
    if(x!=0.0)
        sum += (1.0/PI)*(cos(PI*x)*ln(abs(-x))-cos(PI*x)*ci(PI*(-x)))
    sum+= (1.0/PI)*(sin(PI*x)*si(PI*(-x)))
    return  sum
}

fun fununderint3(a:Double,b:Double):Double{
    return fpervisna3(b)-fpervisna3(a)
}

fun fununderint4(a:Double,b:Double):Double{
    return fpervisna4(b)-fpervisna4(a)
}
fun fpervisna4(x:Double):Double{
    var sum = 0.0
    if(x!=0.0)
        sum += (6*Math.pow(x,4.0)-12*Math.pow(x,3.0))*ln(abs(x))
    if(x!=1.0)
        sum += -6*ln(abs(x-1))*Math.pow(x,4.0)+12*Math.pow(x,3.0)*ln(abs(x-1))-12*x*ln(abs(x-1))+6*ln(abs(x-1))
    sum+= -6*Math.pow(x,3.0)+9*x*x+16*x
    return  sum.toDouble()/72.0
}
...
fun getPohubka(a:DoubleArray,b:DoubleArray):Double{
    ...
    return max//Math.sqrt(sum)/a.size.toDouble()
}